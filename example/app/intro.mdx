...
import Dynamic from "../examples/dynamic/index.tsx";
...
`Item` is forwarded from `@react-stately/collections`. See their [Collection API][collection] for an in-depth overview covering this component's API.
...
`CarouselAutoplayControl` does not have any unique props, and accepts any valid `HTMLButtonElement` prop.
